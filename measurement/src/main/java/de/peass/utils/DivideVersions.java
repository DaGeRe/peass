               ...
               for (final Map.Entry<ChangedEntity, List<String>> testcase : changedTests.getVersions().get(endversion).getTestcases().entrySet()) {
                  for (final String method : testcase.getValue()) {
                     ...
                     writer.write("java -cp target/measurement-0.1-SNAPSHOT.jar de.peran.AdaptiveTestStarter "
                           ...
                           + "-executionfile ../dependency/results/execute-" + VersionComparator.getProjectName() + ".json "
                           + "-folder ../../projekte/" + VersionComparator.getProjectName() + "/ "
                           + "-dependencyfile ../dependency/results/deps_" + VersionComparator.getProjectName() + ".xml &> measurement_" + endversion.substring(0, 6) + "_" + testcaseName
                           + ".txt\n");
                     writer.flush();
                  }
               }
