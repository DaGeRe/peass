...
mkdir -p $PEASSHOME"performanzanalysator/dependency/results"
...
tmux send-keys -t $P2 -l "java -Xmx20g -jar target/dependency-0.1-SNAPSHOT.jar -folder ../../projekte/$FOLDER 2>&1 | tee results/depanalyse_$FOLDER.txt && java -Xmx20g -cp target/dependency-0.1-SNAPSHOT.jar de.peran.ViewPrintStarter -dependencyfile results/deps_$FOLDER.xml -folder ../../projekte/$FOLDER 2>&1 | tee results/viewanalyse_$FOLDER.txt"
